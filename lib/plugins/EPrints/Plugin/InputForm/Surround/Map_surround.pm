...
EPrints::Plugin::InputForm::Surround::Default_map

Create to add map layout elements for google maps to hook to

=cut

package EPrints::Plugin::InputForm::Surround::Map_surround;
...
sub render
{
	...
	# add a hook for the google maps canvas
	my $map_frame = $self->{session}->make_element( "div", id => "ep_map_frame" );
	my $map_search = $self->{session}->make_element( "input", id => "pac-input", class=>"controls", type=>"text", placeholder=>"Search Box");
	my $map_div = $self->{session}->make_element( "div", id => "map-canvas" );
	my $info = $self->{session}->make_element( "div", id => "click_info" );
	
	$map_frame->appendChild( $map_search );
	$map_frame->appendChild( $map_div );
	$map_frame->appendChild( $info );

	my $content_inner = $self->{session}->make_element( "div", id => $component->{prefix}."_content_inner" );
	
	$surround->appendChild( $title_bar );
	
	# add the map frame to the surround component to decouple from the Field::Multi width restrictions
	# placed on the help
	$surround->appendChild( $map_frame );
